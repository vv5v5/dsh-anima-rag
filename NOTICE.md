...
| 内容 | 状态 |
|---|---|
| 检索核心（向量检索、策略步、rerank 拦截） | ✅ 移植并按 DSH 重写 |
| echo（回响）机制 | ✅ 移植 |
| BM25 / minisearch 第二轨（上游有这一轨） | ⛔ **未移植 / 已退役** —— 2026-09-26 从本仓整条摘除（索引构建、读侧打分、配置段与面板字段一起摘） |
...
- **依赖**：`vectra` / `adm-zip` / `js-yaml` / `zod` / `patch-package`
  —— 均为各自作者的作品（多为 MIT），按各自许可使用；本仓库不重新分发它们的源码（由 npm 安装）。
  （★ 2026-09-26：`minisearch` 与 `jieba-wasm` 随 BM25 支线退役，已从 `package.json` 摘掉。）
