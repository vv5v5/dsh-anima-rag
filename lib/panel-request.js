...
export function explainRetrieval(p = {}) {
  ...
  // ★ 2026-09-26（T1）：**读侧故障排第一** —— 它比"隔离""库不存在"更根本：
  //   连嵌入都没打通时，后面那些解释都是次要的（而且它一定意味着本轮注入是空的）。
  const failure = p.failure && typeof p.failure === 'object' ? p.failure : null
  if (failure !== null) {
    const label = String(failure.label ?? failure.code ?? '未知故障')
    const bits = []
    if (Number.isFinite(failure.attempts) && failure.attempts > 0) bits.push(`已试 ${failure.attempts} 次`)
    if (Number.isFinite(failure.elapsedMs) && failure.elapsedMs > 0) bits.push(`${(failure.elapsedMs / 1000).toFixed(1)}s`)
    out.push(`**读侧检索没跑成**：${label}${bits.length > 0 ? `（${bits.join('，')}）` : ''}`
      + `${failure.detail ? ` —— ${String(failure.detail).slice(0, 120)}` : ''}`
      + '。这一轮注入的是**故障说明**（不是记忆）；端点/模型在「向量检索 API」那张卡里改。')
  }
  if (iso && iso.blocked) {
