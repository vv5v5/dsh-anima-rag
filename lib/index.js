...
import { buildSessionIndex, resolvePlaythroughForSession } from './session-playthrough.js'
import {
...
export function applyRetrievalConfig(cfg, mtRetrieval) {
  ...
  //   ⛔ 必须在下面那道"四项全空就早退"之前处理：只拨开关、没填 URL 时不能被吞掉。
  const chatEnabledOff = r.chatEnabled === false
  const othersEmpty = url === '' && model === '' && rerankModel === '' && key === ''
...
export function apply(ctx, rawConfig) {
  ...
  function isolationPlan(sessionId) {
    ...
    // ★ 2026-09-19（路线 2）：**会话优先** —— 拿活跃会话去 Tavern 的 catalog/timeline 里查它属于哪个周目；
    //   查不到（没有会话身份 / 不是 Tavern 的会话 / catalog 读不到）⇒ 回落面板绑定值（= 改前的语义，
    //   ⛔ 绝不因为"解析不出"就变成不过滤）。`boundSource` 如实标注判词是谁给的，排障一眼可见。
    const resolved = resolvePlaythroughForSession(sessionPlaythroughIndex(), sessionId, boundPlaythroughId())
  ...
  async function retrieve({ searchText, bm25SearchText, sessionKey }) {
    ...
    try {
      ...
      const outText = ensuredNote === '' ? text : (text === '' ? ensuredNote : `${ensuredNote}\n\n${text}`)
      const out = {
        text: outText,
        stats: {
          ...s,
  ...
  function ingestFileNamesFor(sessionId) {
    const dir = ingestDirFor(sessionId)
    if (dir === '') return []
  ...
  /**
   * 执行面板动作。**只在这里**做写操作；四个动作都返回 `{ok, message, counts?}`。
   ...
   */
  /**
   * 「这次该入哪个目录」—— 写侧跟**活跃会话的周目**（2026-09-19 路线 2）：
   *   会话解析得出就用它；解析不出（没会话身份 / catalog 读不到 / 该会话不在 catalog 里）⇒ **回落面板绑定值**。
   * ⛔ 空串 = 拿不到目录（调用方据此早退）。
   * ★ 抽出来给 `autoIngestOnce` 与 `hasPendingIngestWork` **共用**，⛔ 不许两处各写一遍（会漂）。
   */
  function ingestDirFor(sessionId) {
    const idx = sessionPlaythroughIndex()
    const resolved = resolvePlaythroughForSession(idx, sessionId, boundPlaythroughId())
    const meta = resolved.source === 'session' && idx !== null && idx.byId instanceof Map
      ? (idx.byId.get(resolved.bound) ?? null)
      : null
    const scoped = meta !== null ? summariesDirFor(meta.characterId, resolved.bound) : ''
    return scoped !== '' ? scoped : resolveSummariesDir()
  }
  ...
  async function autoIngestOnce(sessionId, opts = {}) {
    ...
    writeIngestState({
      ...
      source: resolved.source, pending: pending.length,
    })
  ...
  function kickAutoIngest(sessionId) {
    ...
    //   以前这里**每轮装配**都会：置 running → 写 ingest-state → 跑一次 → 发现没变 → 收工；
    //   于是面板那条"记忆库正在后台收纳"的提示**每轮闪一次**（明明什么都没入）。
    //   现在：**没有新东西就根本不启动**（连 running 都不置、状态文件都不写）⇒
    //   提示只在"真入库"那一刻出现，而摘要只由**压缩**产出 ⇒ 效果就是"压缩时才触发"。
    //   ⚠️ 唯一例外：本进程的第一脚仍然照跑 —— 那一次是为了 `autoMaintain()` 的孤儿清理
    //     （它每进程只做一次，⛔ 不能因为"摘要没变"就永远不做）。
    const firstOfProcess = !maintainDone
    if (!firstOfProcess && !hasPendingIngestWork(sessionId)) return
