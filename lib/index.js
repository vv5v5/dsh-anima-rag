 ...
 *
 * @param {{env?: object, warn?: (msg: string) => void}} [opts]
 *   `env` 可注入（自检用；默认 process.env）。key 仍空而环境变量有 ⇒ 沿用 env 兜底
 *   （**过渡态**：正确落点是记忆库设置里的「向量检索 API」卡），命中时调 `warn` 大声提醒。
 */
export function mergeConfig(rawConfig, opts) {
  ...
  if (!cfg.embed.key && env.ANIMA_RAG_EMBED_KEY) {
    cfg.embed.key = env.ANIMA_RAG_EMBED_KEY
    if (say) say('正在用环境变量供 embed key —— 这是过渡态，请在记忆库设置里配一次')
  }
  if (!cfg.rerank.api.key && env.ANIMA_RAG_RERANK_KEY) {
    cfg.rerank.api.key = env.ANIMA_RAG_RERANK_KEY
    if (say) say('正在用环境变量供 rerank key —— 这是过渡态，请在记忆库设置里配一次')
  }
  return cfg
}
...
export function apply(ctx, rawConfig) {
  ...
  // 生效顺序只有两层：记忆库 retrieval.* → DEFAULTS。记忆库已配了 key ⇒ 连环境变量都不看
  // （mergeConfig 的 env 兜底不触发，也就不会误报"过渡态"）。
  const mtHasKey = mtRetrieval !== null && typeof mtRetrieval.key === 'string' && mtRetrieval.key.trim() !== ''
  const cfg = applyRetrievalConfig(
    mergeConfig(rawPlugin, { env: mtHasKey ? {} : process.env, warn: (m) => warn(ctx, m) }),
    mtRetrieval,
  )
  ...
  if (!cfg.embed.key) warn(ctx, 'embed.key 未配置 —— 向量支线会失败（BM25 支线仍可用）。请在记忆库设置（「向量检索 API」卡）里配一次；环境变量 ANIMA_RAG_EMBED_KEY 只是过渡态兜底。')
}
