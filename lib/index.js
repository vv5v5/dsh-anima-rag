 ...
 */
export function mergeConfig(rawConfig) {
  const raw = rawConfig ?? {}
  ...
  if (!cfg.embed.key && process.env.ANIMA_RAG_EMBED_KEY) cfg.embed.key = process.env.ANIMA_RAG_EMBED_KEY
  if (!cfg.rerank.api.key && process.env.ANIMA_RAG_RERANK_KEY) cfg.rerank.api.key = process.env.ANIMA_RAG_RERANK_KEY
  return cfg
}
 ...
 */
export function composePayload(cfg, { searchText, bm25SearchText, sessionKey }) {
  const primaryChat = cfg.chatCollections?.[0]
  const chatStrategy = {
  ...
  return {
    ...
    chatContext: { ids: cfg.chatCollections, strategy: chatStrategy },
    ...
    bm25Configs: {
      chat: cfg.chatCollections.map(collectionId => ({ collectionId, dictionary: cfg.bm25.dictionary })),
      kb: [],
...
export function apply(ctx, rawConfig) {
  const cfg = mergeConfig(rawConfig ?? ctx?.config)
  ...
  /** 统计 */
  const stats = { assembles: 0, cacheHits: 0, retrievals: 0, timeouts: 0, errors: 0, lastError: null, lastQuery: null, lastMs: null, inserts: 0, insertErrors: 0, lastInsert: null, autoIngestRuns: 0, insertRetries: 0 }
  ...
  async function retrieve({ searchText, bm25SearchText, sessionKey }) {
    ...
    try {
      ...
      stats.retrievals += 1
      const payload = buildPayload({ searchText, bm25SearchText, sessionKey })
  ...
  /** 已处理过的来源文件名（成功才记；进程内，重启后最多重入一次 —— 引擎按 index 覆盖）。 */
  ...
  async function autoIngestOnce() {
    ...
    const pending = entries.filter((e) => e && typeof e.file === 'string' && e.file !== ''
      ...
      && !ingestedFiles.has(String(e.file)) && (ingestTries.get(String(e.file)) ?? 0) < 3)
    if (pending.length === 0) return { skipped: 'all-done' }
    const slices = []
    const unreadable = []
    ...
    for (const s of slices) {
      if (okIdx.has(String(s.index))) ingestedFiles.add(String(s.source))
      else ingestTries.set(String(s.source), (ingestTries.get(String(s.source)) ?? 0) + 1)
    }
    stats.autoIngestRuns += 1
    log(ctx, `自动入库：${out.collectionId} 写成功 ${out.inserted} / 失败 ${out.failed}（待处理 ${pending.length} 条）`)
  ...
  ctx.tools.register({
    ...
    async execute(args) {
      ...
      try {
        ...
        const engine = await loadEngine()
        const payload = buildPayload({ searchText, bm25SearchText, sessionKey: typeof args.session_id === 'string' ? args.session_id : (agent?.id ?? 'manual') })
  ...
  if (!cfg.embed.key) warn(ctx, 'embed.key 未配置 —— 向量支线会失败（BM25 支线仍可用）。设 ANIMA_RAG_EMBED_KEY 或写进 profile 配置。')
}
