 ...
 *   硬得多 ⇒ 根**顺序无关、永远赢**；两个都不是根时才按行序（先到者胜）。
 *   真机事故（就是这条逼出来的）：一条会话同时是**影子·12周目**的 root、**又是 Rika·1周目** timeline
 *   里的一条 variant 切片 ⇒ 旧的"先到者胜"把它判给了 catalog 里更靠前的 Rika·1周目 ⇒
 *   检索目标/归档楼层/笔记目录**全指到一个空周目**。
 *
...
/**
 * ★★ 2026-09-27（用户口径：「**修逻辑。让anima能认出活跃会话的性质**」）：会话白名单判定（纯函数）。
 *
 * 解决什么：注入白名单原先**只认 UI 标记**（Tavern `session-selections.json` 的
 * `rp.active=true`，source=`character-follow`）——而「与 X 新开周目」/继续/回档开的会话
 * **不被那些流程打标** ⇒ 只要名单里有任何一个别的会话带着标记，新周目会话就被**静默滤掉**
 * （检索整个不跑、零日志零故障说明；真机 12 周目踩过）。
 *
 * 新识别路径：**看会话的性质，不只看 UI 标记** —— Tavern catalog/timeline 里**绑定了周目**
 * 的会话（`resolvePlaythroughForSession(...).source === 'session'`），本质上就是 RP 周目会话
 * （这条机器与周目隔离闸用的是同一份）。编程/工具会话**从不**绑定周目 ⇒ 防污染的初衷不变。
 *
 * @param {object} input
 *   rpOnly           - config `inject.rpOnly`（严格 false 才关）。
 *   rpMarked         - 本会话在 rp.active 集合里（UI 标记路径）。
 *   rpMarkedCount    - rp.active 集合的**总数**（>0 = 名单激活）。
 *   allowSessions    - config `inject.allowSessions`（显式名单，兜底/覆盖）。
 *   playthroughBound - 本会话经 catalog/timeline 解析出绑定周目（**性质**路径）。
 *   sessionId        - 会话 id（allowSessions 成员判定用）。
 * @returns {{ allowed: boolean, reason: string }} reason ∈
 *   'rp-only-off' | 'rp-marked' | 'allow-list' | 'playthrough-bound' | 'not-rp' | 'no-criteria'
 */
export function decideSessionInject(input) {
  const i = input ?? {}
  const rpOnly = i.rpOnly !== false
  if (!rpOnly) return { allowed: true, reason: 'rp-only-off' }
  if (i.rpMarked === true) return { allowed: true, reason: 'rp-marked' }
  const inList = Array.isArray(i.allowSessions) && typeof i.sessionId === 'string'
    && i.allowSessions.includes(i.sessionId)
  if (Number(i.rpMarkedCount) > 0) {
    if (inList) return { allowed: true, reason: 'allow-list' }
    // ★ 性质路径：绑定了周目 = RP 周目会话（⛔ 这一行就是本次修的逻辑）
    if (i.playthroughBound === true) return { allowed: true, reason: 'playthrough-bound' }
    return { allowed: false, reason: 'not-rp' }
  }
  // 名单空（Tavern 未装 / 全无标记）⇒ 回退旧行为 + 显式名单
  if (!Array.isArray(i.allowSessions) || i.allowSessions.length === 0) {
    return { allowed: true, reason: 'no-criteria' }
  }
  return inList ? { allowed: true, reason: 'allow-list' } : { allowed: false, reason: 'not-rp' }
}
