...
export function createEngine(options = {}) {
    ...
    /** 把一次失败归类成机器码（读侧要按它写"如实说明"，⛔ 不许只留一句人话字符串）。 */
    function classifyEmbeddingError(error) {
        const name = String(error?.name ?? "");
        if (name === "AbortError" || name === "TimeoutError")
            return "TIMEOUT";
        if (name === "TypeError" || error?.cause) return "NETWORK";
        return "UNKNOWN";
    }

    /** 一次尝试：拿到向量就返回，否则抛**带 `code`** 的错（调用方决定要不要再来一次）。 */
    async function embedOnce(text, config, fetchUrl, waitMs) {
        const controller = new AbortController();
        ...
        try {
            ...
            if (!data.data[0].embedding) {
                const e = new Error("向量 API 返回了数据，但未找到 embedding 字段。");
                e.code = "SHAPE";
                e.elapsedMs = Date.now() - t0;
                throw e;
            }
            ...
            clearTimeout(timeoutId);
            const code = classifyEmbeddingError(error);
            if (code === "TIMEOUT") {
                // 精准拦截超时错误，转化为明确的文字报错
                logger.error(
            ...
            if (code === "NETWORK") {
                ...
                const e = new Error(
                    `嵌入端点连不上：${String(error?.cause?.message ?? error?.message ?? error).slice(0, 120)}`,
                );
                e.code = "NETWORK";
                e.elapsedMs = Date.now() - t0;
                throw e;
            ...
            if (error && typeof error === "object" && !error.code) {
                error.code = "UNKNOWN";
                if (!Number.isFinite(error.elapsedMs)) error.elapsedMs = Date.now() - t0;
            }
            throw error;
        }
    }
    ...
    //
    // ★★ 2026-09-26（T1「嵌入超时当一等故障」）：这一脚现在**必定在预算内收场** ——
    //    要么给出向量，要么抛一个**带 `code` 的错**（调用方据此在注入那一格写如实说明）。
    //
    // 三个旋钮（全部可配，读侧的取值与理由写在 `lib/index.js` 的 DEFAULTS.embed 注释里）：
    //   · `config.timeout_ms` —— **单次尝试**的上限（超时即 abort，⛔ 不无限挂）
    //   · `config.attempts`   —— 总尝试次数（默认 1 ⇒ 与改前行为一致；**只对超时/网络错重试**，
    //                           4xx/shape 错重试没意义，一次就抛）
    //   · `config.deadlineMs` —— **绝对截止时刻**（读侧按"每轮检索预算"算出来）。
    //                           换来的性质：嵌入阶段永远先于轮预算结束 ⇒
    //                           ⛔ 再也不会出现改前那种「fetch 还在跑、这一轮已经放弃它了」
    //                           （结果算完没人要、还白烧一次调用）。
    async function getEmbedding(text, config) {
        ...
        const perAttempt = Number(config.timeout_ms) > 0 ? Number(config.timeout_ms) : 15000;
        const attempts = Math.max(1, Math.min(3, Math.trunc(Number(config.attempts)) || 1));
        const deadlineMs = Number(config.deadlineMs) > 0 ? Number(config.deadlineMs) : 0;
        ...
        for (let attempt = 1; attempt <= attempts; attempt += 1) {
            const remain = deadlineMs > 0 ? deadlineMs - Date.now() : Infinity;
            // 预算不够再起一次（起步就 <600ms ⇒ 必超时，重试只是白烧）⇒ 用上一次的错误收场
            if (attempt > 1 && remain < 600) break;
            const waitMs = remain === Infinity
                ? perAttempt
                : Math.max(300, Math.min(perAttempt, remain));
            tried = attempt;
            try {
                return await embedOnce(text, config, fetchUrl, waitMs);
            } catch (error) {
                lastErr = error;
                ...
                logger.log(
                    `[Anima RAG] ↻ 嵌入${error.code === "TIMEOUT" ? "超时" : "网络错"} ⇒ 重试 ${attempt + 1}/${attempts}（单次上限 ${waitMs}ms）`,
                );
            }
        }
        // 收场：把"试了几次 / 一共花了多久 / 打的是哪个端点与模型"补齐 —— 读侧那句如实说明要用。
        const e = lastErr instanceof Error ? lastErr : new Error(String(lastErr ?? "嵌入失败"));
        e.endpoint = fetchUrl;
        e.model = String(config.model ?? "");
        e.attempts = tried;
    ...
    /**
     * ★★ 2026-09-26（T1）：查询向量 vs「各库里存量向量」的**维度核对** —— 只读、只报，⛔ 不改结果。
     *
     * 为什么要它：维度不一致时相似度会**静默**变成 NaN / 垃圾值
     * （`lib/index.js` 的 `DEFAULTS.embed` 注释里那条老警告：「换维度/换模型必须整库重算」，
     *  真机实测 2026-09-26：`dsh-memory` 是 4096 维、其余 12 个库是 2560 维 ⇒ 那 12 个库
     *  每条命中的 `score` 都是 `NaN`，而检索照样返回、界面照样"有结果"）。
     *
     * ⛔ 这里**不过滤** NaN 结果：过滤会让"库还在但排不出序"变成"看起来库里没东西"，
     *    反而是更难查的静默。诊断只把原因写进 `_diag`，由读侧决定怎么**如实说**。
     */
    async function dimensionDiagnostics(vector, chatContext, kbContext) {
        const out = {
            query: Array.isArray(vector) ? vector.length : null,
            collections: [],
            mismatched: [],
        };
        if (!out.query) return out;
        const ids = [
            ...(Array.isArray(chatContext?.ids) ? chatContext.ids : []),
            ...(Array.isArray(kbContext?.ids) ? kbContext.ids : []),
        ].filter((id) => typeof id === "string" && id !== "");
        for (const id of ids) {
            try {
                const idx = await getIndex(id, false);
                if (!idx) continue;
                const items = await idx.listItems();
                const stored = Array.isArray(items) && items[0]?.vector
                ...
                if (stored !== null && stored !== out.query)
                    out.mismatched.push(`${id}（库 ${stored} 维 / 查询 ${out.query} 维）`);
            } catch {
                // 读不到就跳过：⛔ 诊断本身绝不许把检索弄坏（它只是"说明"，不是"判据"）
            }
        }
        return out;
    }
    ...
    async function query(payload = {}) {
        try {
            ...
            // ★ 2026-09-26（T1）：维度核对放在**结果都算完之后**、且在 try 里 ——
            //   它只做一次只读的 `listItems()`（索引本就为检索加载过，是内存里的现成数据）。
            //   ⛔ 它抛错也绝不能让整轮检索失败（外面那层 catch 会把整轮判成 ANIMA_QUERY_FAILED）。
            let dimDiag = { query: Array.isArray(vector) ? vector.length : null, collections: [], mismatched: [] };
            try {
                dimDiag = await dimensionDiagnostics(vector, chatContext, kbContext);
            } catch (e) {
                logger.error("[Anima RAG] 维度核对跳过（不影响本轮结果）:", e?.message ?? e);
            }
            ...
            return {
                ...
                // [给"为什么不注入/为什么像空的"用] 2026-09-26：查询向量与库存向量的维度核对
                _diag: dimDiag,
            };
        } catch (err) {
            logger.error(err);
            ...
            // ★★ 2026-09-26（T1「嵌入超时当一等故障」）：**把底层的分类原样带出来**。
            //   ⛔ 改前这一层只留一句 message ⇒ 读侧拿不到"到底是超时、连不上、还是没配 key"，
            //   只能写一句无用的"未知错误"；而"读侧如实说明"正是本单要的那件事。
            //   `originCode` = 底层那个码（TIMEOUT / NETWORK / HTTP / SHAPE / NO_KEY）；
            //   下面几个诊断字段一并搬过来（读侧那句说明里的"试了几次 / 花了多久 / 打哪个端点"）。
            //   ⚠️ `code` 本身**保持 `ANIMA_QUERY_FAILED` 不变**（那是对外契约，别动它）。
            wrapped.cause = err;
            if (err && typeof err === "object") {
                wrapped.originCode = typeof err.code === "string" ? err.code : null;
                for (const k of ["attempts", "elapsedMs", "endpoint", "model"]) {
                    if (err[k] !== undefined && err[k] !== null) wrapped[k] = err[k];
                }
            }
            throw wrapped;
        }
