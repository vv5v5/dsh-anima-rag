...
**向量检索**、策略步与 rerank 拦截、echo（本地回响），检索结果经 `system-prompt/assemble`
瀑布注入 RP 会话；既是**独立的检索工具**，也是这套酒馆 agent 的**记忆库检索端**。

> ★ 2026-09-26：上游那份是「向量 + BM25 双轨」，**BM25（minisearch）那条支线已于本日整条退役**
> （用户口径「fts5目前看用处不大，摘除吧」）—— 索引构建、读侧打分、`data/bm25Root` 配置段、
> `minisearch` / `jieba-wasm` 依赖、面板的「删除 BM25 库」动作与状态字段**全部摘掉**。
> ➜ **检索只剩向量一条路**（没有备用轨：嵌入渠道不通 ⇒ 检索整个不通，读侧会如实说明）。
...
| | |
|---|---|
| **检索** | 每轮装配时按会话取词 → 向量检索 → 策略步（base / important / diversity）→ rerank 拦截 → 注入 system 段 `anima:memory` |
| **检索工具** | `anima_query`：模型可**主动**查历史（返回命中条数、正文、以及**为什么是 0 命中**的一行诊断） |
| **入库** | 把**摘要目录**里的摘要写进记忆库（写向量，写完回读核对）：压缩后自动入库，也可手动触发 |
...
| **向量面板的那半边** | 读 `dsh-memory-archive` 写来的**请求单**执行动作（立即入库 / 重建（向量）/ 删向量库），并把**状态快照**与**回执**写回去给它显示 |

**向量生成的时机**（明确只有两处）：① **自动压缩入库时**（摘要进库 ⇒ 写向量）；
...
数据根默认**指向 ST 侧现役 Anima 的数据**（`vectors/` + `data/sessions/`）——
...
- **检索核心**：向量检索、策略步、rerank 拦截
  （上游那份还有 BM25 第二轨；**本仓那条轨 2026-09-26 已摘除**，见文件开头那段）
- **echo（回响）机制**：上游 `processEchoLogic` 逐字搬运（它在引擎内部跑，
  候选池 = 本轮的向量命中；BM25 退役后这一点没有变）
